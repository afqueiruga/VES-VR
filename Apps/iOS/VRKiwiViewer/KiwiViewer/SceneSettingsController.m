...
+(void)setBackgroundColor:(vesSharedPtr<kiwiApp>)app
{
  ...
  vesVector3f bottomColor;
    CGFloat dtopColor[3];
    CGFloat dbottomColor[3];
  [backgroundColor getRed:&(dbottomColor[0]) green:&(dbottomColor[1]) blue:&(dbottomColor[2]) alpha:NULL];
    
    bottomColor[0] = dbottomColor[0];
    bottomColor[1] = dbottomColor[1];
    bottomColor[2] = dbottomColor[2];
    // This is a 64bit bug it seems?
  if (useGradient) {
    [backgroundColor2 getRed:&dtopColor[0] green:&dtopColor[1] blue:&dtopColor[2] alpha:NULL];
      topColor[0] = dtopColor[0];
      topColor[1] = dtopColor[1];
      topColor[2] = dtopColor[2];
  }
  else {
